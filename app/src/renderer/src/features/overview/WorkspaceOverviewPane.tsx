...
const SERIES_COLORS = ["var(--accent)", "var(--success)", "var(--warning)", "var(--danger)", "var(--info)", "var(--muted)"];
...
export function WorkspaceOverviewPane({ tabs, models, overview, onChangeOverview }: Props) {
  ...
  const modelRows = models.map((model) => {
    ...
    return { id: model.id, label: model.label, values, overall: average(valid), completeness: `${valid.length}/${selectedTabs.length}` };
  });
    ...
    <div className="tabbed-workspace-pane is-active workspace-overview">
        ...
        {loading ? <div className="overview-empty"><span className="spinner" /> Loading scores...</div> : (
          <div className="table-scroll">
            <table className="overview-score-table">
              <thead><tr><th>Radar</th><th>Model</th>{selectedTabs.map((tab) => <th key={tab.id}>{tab.title}</th>)}<th>Overall</th><th>Coverage</th></tr></thead>
              <tbody>{rows.map((row) => (
                <tr key={row.id} className={row.id === AVERAGE_ID ? "is-average" : ""}>
                  ...
                  <td>{formatScore(row.overall)}</td><td>{row.completeness}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
      ...
      <section className="overview-section overview-chart-section">
        ...
        {selectedTabs.length < 3 ? <div className="overview-empty">Select at least 3 tabs to display the radar chart.</div> : chartRows.length === 0 ? <div className="overview-empty">Select at least one model or the all-model average.</div> : (
          <RadarChart tabs={selectedTabs} rows={chartRows} />
        )}
      </section>
    </div>
  );
...
function RadarChart({ tabs, rows }: { tabs: BenchLocalWorkspaceTab[]; rows: Array<{ id: string; label: string; values: Array<number | null> }> }) {
  ...
  const ring = (scale: number) => tabs.map((_, index) => point(index, scale).join(",")).join(" ");
  return <div className="overview-radar-wrap">
    <svg className="overview-radar" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Radar comparison of selected models across selected evaluation tabs">
      ...
      {[20,40,60,80,100].map((scale) => <polygon key={scale} points={ring(scale)} className="radar-grid" />)}
      {tabs.map((tab, index) => { const end = point(index, 100); const label = point(index, 118); return <g key={tab.id}><line x1={cx} y1={cy} x2={end[0]} y2={end[1]} className="radar-axis"/><text x={label[0]} y={label[1]} textAnchor={label[0] < cx - 10 ? "end" : label[0] > cx + 10 ? "start" : "middle"} className="radar-label">{tab.title}</text></g>; })}
      {rows.map((row, rowIndex) => {
        const points = row.values.map((value, index) => point(index, value ?? 0));
