...
describe("Agent API contract", () => {
  it("keeps the documented HTTP and MCP entrypoints wired", async () => {
    const [agentServerSource, agentMcpSource, mcpRouterSource, mcpReadToolsSource, mcpWriteToolsSource, mcpResourcesSource, capabilitySource] = await Promise.all([
    ...
    const mcpTools = new Set(readStringMatches(`${mcpReadToolsSource}\n${mcpWriteToolsSource}\n${capabilitySource}`, /"(benchlocal_[^"]+)"/g));
    ...
    expect(mcpRouterSource).not.toContain("server.registerTool(");
    expect(mcpRouterSource).toContain("registerBenchLocalMcpWriteTools(server, writeCapabilities)");
    expect(mcpWriteToolsSource).toContain("server.registerTool(");
